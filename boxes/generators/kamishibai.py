...
class Kamishibai(_TopEdge):
    ...
    def __init__(self) -> None:
        ...
        hinges_3panes__group = self.argparser.add_argument_group("Kamishibai 3 pane cover hinge holes")
        hinges_3panes__group.add_argument(
    ...
    def boxFrontBackCallback (self, wi, hi, isFront):
        ...
        if self.HandleThickness > 0 :
            ...
            self.fingerHolesAt(wi/2 , hi - self.thickness * 1.5, self.thickness*4,0)
    ...
    def boxTopBottom (self, wi, di, isTop, move=None, label=""):
        ...
        if (isTop and self.HandleThickness > 0 and self.HandleWidth > 0) :
            self.rectangularHole(self.thickness*4 + wi/2 - self.HandleWidth/2, self.thickness * (self.BackExtraDepth + 3 - self.HandleThickness/2) + di/2, self.HandleWidth, self.thickness*self.HandleThickness, 0, False, False)
        # hinge holes for 3 pane pannel
    ...
    def topHandle (self, wi, di, move=None, label=""):
        ...
        self.edge(wi/2 - self.thickness*6)
        self.edges["f"](self.thickness*4)
        self.edge(wi/2 - self.thickness*6)
        self.edges["f"](self.thickness*4)
        self.corner(90)
    ...
    def lockSimple (self, move=None):
        ...
        # discs attachement
        self.rectangularWall(self.thickness*4, self.thickness, "eeee", move=move)
    ...
    def render(self):
       ...
       if self.FrontCoverStyle=="two-part lid with hinge eyes (both ends)":
           # front panel sides
           self.coverPanel2Side(wi, hi, "none", move="mirror up", label="front panel right")
       ...
       # three-part lid, higes not provided
           ...
           # front panel sides
           self.coverPanel3Side(wi, hi, move="mirror up", label="front panel right")
           self.coverPanel3Side(wi, hi, move="up", label="front panel left")
           # front panel top
